...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('    fi')
    # Accept any of unzip / bsdtar / tar / python3 for prebuilt extraction.
    # New llama.cpp releases may publish `.tar.gz` artifacts instead of `.zip`.
    runner_lines.append('    if [ -n "$_odysseus_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v tar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    ...
    runner_lines.append('      _odysseus_archive="llama-cpp-prebuilt.tar.gz"')
    runner_lines.append('      case "$_odysseus_prebuilt_url" in')
    runner_lines.append('        *.zip) _odysseus_archive="llama-cpp-prebuilt.zip" ;;')
    runner_lines.append('        *.tar.gz|*.tgz) _odysseus_archive="llama-cpp-prebuilt.tar.gz" ;;')
    runner_lines.append('      esac')
    runner_lines.append('      rm -f llama-cpp-prebuilt.tar.gz llama-cpp-prebuilt.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_odysseus_prebuilt_url" -o "$_odysseus_archive" && [ -s "$_odysseus_archive" ]; then')
    runner_lines.append('        rm -rf build && mkdir -p build')
    runner_lines.append('        if echo "$_odysseus_archive" | grep -Eq "\\\\.tar\\\\.gz$|\\\\.tgz$"; then')
    runner_lines.append('          if command -v bsdtar >/dev/null 2>&1; then bsdtar -xf "$_odysseus_archive" -C build; elif command -v tar >/dev/null 2>&1; then tar -xzf "$_odysseus_archive" -C build; fi')
    runner_lines.append('        else')
    runner_lines.append('          if command -v unzip >/dev/null 2>&1; then unzip -qq -o "$_odysseus_archive" -d build; elif command -v bsdtar >/dev/null 2>&1; then bsdtar -xf "$_odysseus_archive" -C build; else python3 -c "import zipfile; zipfile.ZipFile(\\\\\\"$_odysseus_archive\\\\\\").extractall(\\\\\\"build\\\\\\")"; fi')
    runner_lines.append('        fi')
    ...
    runner_lines.append('          # Resolve to an absolute path so the ~/bin/llama-server symlink does')
    runner_lines.append('          # not dangle: find returns a path relative to the cache dir we cd into')
    runner_lines.append('          # above, but the symlink is read from ~/bin, so a relative target would')
    runner_lines.append('          # resolve against the wrong directory.')
    runner_lines.append('          _odysseus_extracted="$(readlink -f "$_odysseus_extracted" 2>/dev/null || echo "$(pwd)/$_odysseus_extracted")"')
    ...
    runner_lines.append('      echo "[odysseus]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[odysseus]   Install Vulkan (libvulkan-dev + glslc) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
